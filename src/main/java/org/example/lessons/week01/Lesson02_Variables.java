...
public class Lesson02_Variables {
    ...
    private static void practiceVariables() {
        ...
        System.out.println("Когда я родился: " + birthYear);
        System.out.println("Сколько мне лет: " + age);
        ...
        System.out.println("Возраст: " + agePrimary + " лет");
        System.out.println("Рост: " + height + " м ");
        System.out.println("Студент: " + (isStudent ? "Да" : "Нет"));
        System.out.println("Моя оценка: " + grade);
        ...
        System.out.println("double " + bigDouble + " -> " + intFromDouble + "(Потеря точности!)");


    }

    public static BigDecimal convertTo(BigDecimal price, Currency from, Currency to) {
        var dollarResult = convertToDollar(price, to);
        var result = convertFromDollar(dollarResult, from);
        System.out.println("Конвертация: " + price + " " + from + " -> " +
                result.setScale(3, RoundingMode.HALF_UP) + " " + to);
        return result;
    }

    public static BigDecimal convertFromDollar(BigDecimal price, Currency from) {
        return price.divide(from.rateToDollar, RoundingMode.HALF_UP);
    }

    public static BigDecimal convertToDollar(BigDecimal price, Currency from) {
        var result = price.multiply(from.rateToDollar);
        return result;
    }
}


        /*  System.out.println("Конвертировано: " + result.setScale(2, RoundingMode.HALF_UP) + " " + to);*/


        /*if(price.compareTo(BigDecimal.valueOf(1000) {
            return;
        }*/

        /*Lesson02_Variables.convertTo(price, Currency.RUB, Currency.USD);*/
